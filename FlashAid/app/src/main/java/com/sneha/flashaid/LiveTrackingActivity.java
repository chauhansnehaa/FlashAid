...
public class LiveTrackingActivity extends AppCompatActivity {
    ...
    private String requestId, driverId, trackingId;
    private double patientLat, patientLon;
    private GeoPoint driverPoint, patientPoint;
    ...
    private void setupMap() {
        ...
        // ✅ Use custom marker icon (optional)
        ...
        startPatientMarkerPulse(patientMarker); // 🔹 Add pulsing animation
    }

    private void fetchDriverMockLocation() {
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot snapshot) {
                        ...
                        if (lat != null && lon != null) {
                            driverPoint = new GeoPoint(lat, lon);
                            addDriverMarker(driverPoint);
                            fetchRoadRoute(lat, lon, patientLat, patientLon);
                        } else {
                            Toast.makeText(LiveTrackingActivity.this, "Driver location unavailable", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
        ...
        new Thread(() -> {
            try {
                ...
                if (responseCode != HttpURLConnection.HTTP_OK) {
                    throw new Exception(
                            "HTTP " + responseCode + " : " + conn.getResponseMessage()
                    );
                }
                ...
                runOnUiThread(() -> startRouteMovement(routePoints));

            } catch (Exception e) {
                runOnUiThread(() ->
                        Toast.makeText(LiveTrackingActivity.this, "Route fetch error: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }
        }).start();
    }
    ...
    private void startRouteMovement(List<GeoPoint> routePoints) {
        ...
        routeLine.setColor(0xAA2196F3);
        mapView.getOverlays().add(routeLine);

        // ✅ Start background service for continuous simulation
        Intent serviceIntent = new Intent(LiveTrackingActivity.this, TrackingService.class);
        ...
        serviceIntent.putExtra("patientLat", patientLat);  // ✅ Added
        serviceIntent.putExtra("patientLon", patientLon);  // ✅ Added
        serviceIntent.putExtra("routePoints", (java.io.Serializable) routePoints);
        startService(serviceIntent);

        // ✅ Add listener to update marker position live from Firebase
        dbRef.child("liveDrivers").child(driverId).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                Double lat = snapshot.child("lat").getValue(Double.class);
                Double lon = snapshot.child("lon").getValue(Double.class);
